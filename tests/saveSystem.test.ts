...
test("save envelope round-trips the whole game state and keeps the state last", () => {
  ...
  assert.ok(text.startsWith('{"schemaVersion":1,'));
...
test("v0 bare states migrate to the v1 envelope", () => {
  ...
  assert.equal(decoded.envelope.schemaVersion, 1);
  ...
  assert.deepEqual(decoded.envelope.state, raw);
});
