#!/usr/bin/env bash
# UX-3R2 gates on the DGX (the UX-3 suite plus the UX-3R2 captures), this build ($URL) beside the trunk before it ($BASE_URL):
#   scripts/remote/run.sh ux3r -- bash scripts/remote/with-base-build.sh <trunk-sha> -- bash scripts/ux3r2Verification.sh
...
step ux3r2-captures node scripts/ux3r2Captures.mjs "$out/ux3r2" --url "$URL"
