"""INSTALL-5c: install the 14 Wave 5c candidates (confirmed in assets-inbox/INBOX_LEDGER.csv) from
assets-inbox/wave5c/candidates-20260925/assets into public/assets, add their ledger rows to docs/provenance/assets.csv
...
BATCH = "assets-inbox/wave5c/candidates-20260925"
...
TARGET = {
    "actors": ("public/assets/walkers-v2", "src/render/walkerSheetManifest.generated.ts (INSTALL-5c child / elder bodies; walkerLook.ts, residentTrips.ts)"),
    "wall": ("public/assets/wall", "src/render/terrainVariantManifest.ts (INSTALL-5c stone gate v3 per axis, 135 degree pillar b / c by turn; gateArtRenderer.ts, drawWallFaces.ts)"),
    "water": ("public/assets/shore", "src/render/terrainVariantManifest.ts (INSTALL-5c shallow water d / e; drawShoreline.ts)"),
}
...
def main() -> None:
    ...
    for row in astra:
        ...
        asset_id = name[: -len("-v1.png")]
        prompt = ROOT / "docs/provenance/prompts" / f"{asset_id}-v1.txt"
        ...
        rows.append({
            ...
            "notes": (f"Astra Wave 5c ledger row {row['assetId']} (candidate; confirmed in assets-inbox/INBOX_LEDGER.csv) installed by "
                      f"INSTALL-5c on 2026-09-26 from {BATCH}; received bytes = runtime bytes = the Astra SHA (no caBX). "
                      f"Astra note: {row['notes'][:300]}"),
        })
        installed[f"wave5c/candidates-20260925/assets/{folder}/{name}"] = True
    ...
    for row in inbox_rows:
        if row["file"] in installed:
            row["installed_by"] = "INSTALL-5c"
            row["verdict_note"] = row["verdict_note"].replace("설치 예정 INSTALL-5c(본선 93d0f32 public/assets에서 같은 바이트 미발견)", "INSTALL-5c가 public/assets에 같은 바이트로 설치")
    with open(INBOX_LEDGER, "w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields, lineterminator="\r\n")
        writer.writeheader(); writer.writerows(inbox_rows)
    print(len(rows), "installed;", sum(1 for row in inbox_rows if row["installed_by"] == "INSTALL-5c"), "inbox rows marked")
