...
const BASELINE = JSON.parse(readFileSync("fixtures/ledger/world-baseline-9b5a18c.json", "utf8")) as {
...
for (const [name, expected] of Object.entries(BASELINE.cases)) {
  ...
  test(`L-10 ${name}: the cache matches the ledger on every tick and the money-free world is the pinned C3 world`, async () => {
