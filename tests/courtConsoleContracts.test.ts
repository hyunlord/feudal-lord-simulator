...
test("console CSS uses every generated surface and rejects web-dashboard styling", async () => {
  ...
  assert.match(mobileRules, /--seal-size:\s*46px;/);
  assert.match(mobileRules, /\.build-seals\s*\{[\s\S]*?display:\s*grid;/);
  assert.match(mobileRules, /\.build-seals\s*\{[\s\S]*?grid-template-columns:\s*repeat\(4,\s*minmax\(0,\s*1fr\)\);/);
  assert.match(mobileRules, /\.build-seals\s*\{[\s\S]*?overflow-x:\s*hidden;/);
  assert.match(mobileRules, /\.build-seals\s*\{[\s\S]*?overflow-y:\s*hidden;/);
  assert.match(mobileRules, /\.build-seals\s*\{[\s\S]*?gap:\s*3px 1px;/);
  assert.match(mobileRules, /\.build-group\s*\{[\s\S]*?display:\s*contents;/);
  assert.match(mobileRules, /\.build-group-seals\s*\{[\s\S]*?display:\s*contents;/);
