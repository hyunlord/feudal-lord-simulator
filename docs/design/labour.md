...
- **LB-12 목책 선포 시점**: 자동 성장은 목책을 선포할 때, 제안된 성벽이 정책이 원하는 필지마다 6칸(`wallCellsPerLot`) 이상을 두르는지 본다(성 안 칸 수 ≥ `maxHousingLots` × 6). 모자라면 그 벽으로는 선포하지 않고 기다린다. 도시가 더 자라면 다음 제안이 더 넓게 두른다. 이미 필지를 다 채웠으면 그대로 선포한다.
  - 이유: 식량이 좋아져 목책 조건(인구 60 등)을 중심이 작을 때 채운다. seed 3은 성 안 135칸 벽을 두르고 24번째 집 자리가 나오지 않았다(23필지에서 정지, 관문 ② 회귀). 옛 규칙은 늦게 선포해 147칸이었다. 6은 옛 규칙에서 가장 좁았던 도시의 147 ÷ 24다. 다른 seed의 벽은 215~420칸이라 이 규칙에 걸리지 않는다.
