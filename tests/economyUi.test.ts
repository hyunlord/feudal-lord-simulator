...
test("build groups use explicit containers so neighboring buttons cannot intercept clicks", async () => {
  ...
  assert.match(stylesheet, /@media \(max-width: 900px\) \{[\s\S]*?\.build-seals\s*\{[\s\S]*?display:\s*grid;[\s\S]*?overflow-x:\s*hidden;[\s\S]*?overflow-y:\s*hidden;/);
  assert.match(stylesheet, /@media \(max-width: 900px\) \{[\s\S]*?\.build-group,[\s\S]*?\.road-tool\s*\{[\s\S]*?display:\s*contents;/);
