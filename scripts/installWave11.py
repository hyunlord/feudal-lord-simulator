"""INSTALL-11: install the 55 Wave 11 runtime files (the work order's assets/ = confirmed rows in
assets-inbox/INBOX_LEDGER.csv, assets-inbox/wave11/candidates-v1) into public/assets/wave11/<group>, add one
docs/provenance/assets.csv row each (replacing earlier INSTALL-11 rows) with one prompt file each, fill `installed_by`
in the inbox ledger, and write the render manifest src/render/wave11ArtManifest.generated.ts (size, pivot or ground
anchor from the Astra records). Runtime bytes = received bytes = the Astra SHA.
Run: python3 scripts/installWave11.py <work order assets dir>
"""
import csv
import hashlib
import json
...
BATCHES = ["assets-inbox/wave11/candidates-v1"]
LEDGER = ROOT / "docs/provenance/assets.csv"
INBOX_LEDGER = ROOT / "assets-inbox/INBOX_LEDGER.csv"
MANIFEST = ROOT / "src/render/wave11ArtManifest.generated.ts"
USED_IN = "src/render/wave11ArtManifest.generated.ts (INSTALL-11: construction kits by building family, site props, carpenter / mason walkers, work tools)"
...
def pivot_of(value: str, width: int, height: int) -> dict:
    ...
    if isinstance(data, dict) and isinstance(data.get("suggestedGroundAnchor"), list):
        anchor = data["suggestedGroundAnchor"]  # kits on a reference canvas: the reference's ground point
    ...
    return {"x": 0, "y": 0, "overlay": True}  # drawn on the reference canvas at zero offset
...
def main() -> None:
    ...
    for source_file in sorted(wanted.rglob("*.png")):
        ...
        prompt = ROOT / "docs/provenance/prompts" / f"{group}-{asset_id}-wave11.txt"
        ...
        rows.append({
            ...
            "artBible": "AB_2026-09-19_v1", "historicalProfile": "S_England_1300_1450_v1", "owner": "Astra Wave11", "usedIn": USED_IN,
            "status": "runtime",
            "notes": (f"Astra Wave 11 row {record['asset_id']} (confirmed in assets-inbox/INBOX_LEDGER.csv) installed by INSTALL-11 on "
    ...
    kept = [row for row in csv.DictReader(open(LEDGER, encoding="utf-8")) if not row["runtimePath"].startswith("public/assets/wave11/")]
    ...
    for row in inbox_rows:
        if row["file"] in installed:
            row["installed_by"] = "INSTALL-11"; marked += 1
    ...
    MANIFEST.write_text("// Generated by scripts/installWave11.py — Wave 11 runtime art: size, pivot / ground anchor (Astra registration).\n"
