...
- **SC-11 달력**: `calendar(tick, startYear)`는 `{ year, season, dayOfYear }`를 돌려준다. `TICKS_PER_YEAR = 4000`, 계절은 각 1,000틱(봄·여름·가을·겨울)이다(C1c에서 1,200 → 4,000. 자동 성장 24필지 승리 277k~628k틱이 1369~1457년이 된다). `dayOfYear`는 360일 기준 1~360이며 정수 연산으로 구한다. 틱 0이 시작 연도 봄 1일이다. 잠정값이며 사용자 플레이를 측정한 뒤 조정한다. 달력은 파생값이라 값을 바꿔도 상태는 변하지 않는다. 역사 시대 전이 연도(1315·1337·1348·1380)는 그대로라, 전이 틱이 60,000·148,000·192,000·320,000으로 옮겨진다.
