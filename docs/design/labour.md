...
- **LB-13 관문 ④ 판정(사용자 결정 LB8)**: 원료 부족 ≤ 0.30은 그대로다. 빵은 "안정 구간 끼니 충족 100%(소비 = 요청) + 종료 비축(곳간·곡창·방앗간·집의 밀 + 빵) > 0"으로 판정한다. 빵 생산/요청은 기록만 한다.
  - 이유: 생산/요청은 수요가 다 채워지면 1 + 수출분이 천장이라 여유 지표로 부적합하다. 곡창·방앗간이 빵으로 차면 남는 생산은 시장 수출로만 빠진다. 시장은 석재·목재를 먼저 팔아 빵 수출은 거의 없다.
  - 그래서 비율이 안정 구간 동안 쌓이거나 줄어든 재고에 따라 ±0.04 흔들린다. 옛 규칙은 1.007~1.038, C3 최종 실행은 1.002~1.078이다.
  - 처음 결정(seed마다 옛 값 − 0.01)은 끼니 충족 100%인 seed 1·4에서 그 흔들림 때문에 미달이었다. 그래서 끼니 충족률·비축량으로 대체했다.
...
- **LB-12 목책 선포 벽**: 자동 성장은 목책을 선포할 때 후보 벽 가운데, 정책이 원하는 필지마다 6칸(`wallCellsPerLot`) 이상을 두르는 벽(성 안 칸 수 ≥ `maxHousingLots` × 6)을 먼저 고른다. 이 첫 탐색은 기준 건물·여백 조합을 64번까지 시도하고(예전 8번), 벽마다 서비스 공간 검사는 한 번, 모두 합쳐 8개까지만 한다. 그런 후보가 없으면 예전처럼 처음 받아들여진 벽으로 선포한다(기다리지 않는다).
  - seed 3 선포 직전 상태: 8번 시도로는 135칸 벽 하나뿐이고, 64번이면 156칸 벽이 나온다(1.6초, 선포 때 한 번).
