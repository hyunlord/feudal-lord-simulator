...
- **LB-12 목책 선포 시점**: 자동 성장은 목책을 선포하기 전에, 제안된 성벽 안에 집을 더 놓을 자리(`wallPlotRoom`: 성 안의 빈 풀밭 중 자동 성장의 집 배치 조건을 통과하는 칸)가 정책이 아직 원하는 필지 수(`maxHousingLots − 현재 필지`)의 1.5배(`wallPlotRoomPermille`) 이상인지 본다. 성 안에는 나중에 우물·시장·교회도 들어서므로 여유를 둔다. 모자라면 그 벽으로는 선포하지 않고 기다린다. 도시가 더 자라면 다음 제안이 더 넓게 두른다.
  - 이유: 식량이 좋아져 목책 조건(인구 60 등)을 중심이 작을 때 채운다. 그때 두른 벽(135칸)에 필지가 19~22개만 들어가 seed 3이 멈췄다(옛 규칙은 늦게 선포해 147칸). 자리가 넉넉한 도시에서는 선포 시점이 그대로다(인구 176 fixture: 자리 14 ≥ 남은 9).
