...
- **LB-12 목책 선포 시점**: 자동 성장은 목책을 선포하기 전에, 제안된 성벽 안에 집을 더 놓을 자리(`wallPlotRoom`: 성 안의 빈 풀밭 중 자동 성장의 집 배치 조건을 통과하는 칸)가 정책이 아직 원하는 필지 수(`maxHousingLots − 현재 필지`) 이상인지 본다. 모자라면 그 벽으로는 선포하지 않고 기다린다. 도시가 더 자라면 다음 제안이 더 넓게 두른다.
