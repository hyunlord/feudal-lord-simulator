...
- **F0-A 흐름 뼈대 1 — 압력: 관문 실패, 병합하지 않음(사용자 결정 대기)**(Claude Code, 엔진 세션, 브랜치 `claude/f0a-pressure`만 푸시): [압력 명세](design/flow-pressure.md) FP-1~FP-7, [보고서](verification/f0a-pressure/REPORT.md), 결정 F1~F6(흐름 설계서 권고안 확정)·FP1~FP8.
...
- **F0-A 다음 안**(보고서 "사용자 결정"): × 1.05를 되돌리고 seed 4 운반 정체를 봇에서 고친 뒤 세 번째 가드레일 실행을 승인할지, 관문 ①의 "첫·두 번째 겨울"을 "4년 안"으로 볼지.
