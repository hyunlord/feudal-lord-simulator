...
const DEFAULT_SEED_DIR = "fixtures/determinism";
...
export function canonicalStateHash(state: unknown): string {
  const gameplay = typeof state === "object" && state !== null && !Array.isArray(state)
    ? Object.fromEntries(Object.entries(state).filter(([key]) => key !== "pathCache"))
    : state;
  return createHash("sha256").update(canonicalJson(gameplay)).digest("hex");
}
...
export async function verifySaveDeterminism(args: readonly string[]) {
  ...
  const cases = option(args, "--cases", "seed1,newgame").split(",");
  ...
  try {
    ...
    return { schemaVersion: SAVE_SCHEMA_VERSION, gate: "B8 gate 1: save/load determinism", ticks, newgameTicks, autosaveEveryTicks: AUTOSAVE_EVERY_TICKS,
      hash: "sha256 of canonical (sorted-key) GameState JSON excluding root pathCache", gateMatches, gateCases: rows.length, passed, rows,
      elapsedSeconds: (performance.now() - started) / 1000 };
  } finally {
