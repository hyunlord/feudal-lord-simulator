# 압력 명세 (F0-A) — 계절 결산·배치 장부 예측·실패 사다리 1~2단·첫 겨울나기와 봄 보릿고개·시대 준비도

지시서: F0-A 흐름 뼈대 1 — 압력. 근거: [플레이어 흐름 설계서 v1](player-flow.md) 1절(결정 F1~F6)·2절(루프 5층)·4절(1장 대본, 첫 겨울)·7절(실패 사다리), [보이는 시뮬레이션 설계서 v1](visibility.md) S4(연기 없는 집)·S12(짐 진 가족). 결정: F1~F6(흐름 설계서 권고안 확정), FP1~FP10.
...
- 필드: `season`(0 봄~3 겨울) · `year` · `startTick` · `endTick` · `income` · `expense` · `stockDelta{bread, wheat, timber, stone}` · `popDelta` · `notableEvents[]` · `nextObjectiveHint`.
  ...
  - 다음 목표 힌트: 떠날 준비 가구가 있거나 비축이 한 계절 미만이면 `food_reserve`, 창고와 밭의 식량이 다음 수확까지 못 버티면 `harvest_reserve`(FP-4), 황폐한 집이 있으면 `resettle`, 아니면 없음.
...
## FP-4 첫 겨울나기와 봄 보릿고개 (결정 F4, FP9)

중세 잉글랜드에서 식량이 가장 모자란 때는 겨울이 아니라 이듬해 늦봄~초여름, 지난 수확이 바닥나고 새 수확이 아직 오지 않은 보릿고개였다. 이 게임도 수확이 늦여름~가을이라 겨울은 수확 직후이고, 봇 도시의 모자람은 이듬해 봄~초여름에 온다(관문 ①: 비축 조치를 끈 봇의 1단 5건 중 4건이 봄·여름). 그래서 첫 위기는 "첫 겨울나기와 봄 보릿고개"이고, 경고는 다음 수확까지를 본다.
...
- 목표 카드 훅 `first_winter_warning`: 가을이 시작되는 틱에 창고와 밭의 식량이 다음 수확까지 버티지 못하면 한 번 올린다(`seasons.firstWinterWarning{tick, reserveTicks, untilHarvestTicks}`). `firstWinterWarningActive(state)`는 그 가을과 겨울 동안 참이다.
  - 식량(`harvestOutlookTicks`) = FIX-1 창고 비축 + 돌보는 띠에 선 작물(파종·성장은 다 자란 몫, 여묾은 자란 만큼, 밀 2 = 빵 1)을 보통 끼니 틱으로 잰다.
  - 필요(`ticksUntilNextHarvest`) = 다음 수확 시작(해 안 틱 1,500, AF-11의 비축 전망과 같은 수확 시점)까지의 틱이다. 겨울 틱은 × 1.2로 센다. 가을 시작부터 1,000 + 1,200 + 1,500 = 3,700틱이다.
  - 전에는 겨울 한 계절(1,200틱)만 봤다. 그러면 겨울은 넘기고 봄에 굶는 도시가 경고를 받지 못했다.
...
- 봇의 연 수요 추정은 그대로다(겨울 끼니 × 1.05를 넣은 2회차는 seed 3·5·1·2를 무너뜨려 되돌렸다, 결정 FP6).
- 봇 복구 `barn_mill`(AR-8, 결정 FP10): 헛간에 밀이 밀려 쌓이고 방앗간이 굶는 사이 집이 등급을 잃으면 그 헛간 옆에 방앗간을 짓는다(가드레일 1회차 seed 4).
...
- 관문 ① 판정(결정 FP9): 비축 조치를 끈 봇이 4년(1300~1303) 안에 1단 ≥ 3 seed, 2단 ≥ 2 seed. 모자람이 오는 계절(보릿고개)을 따라 판정 창을 "첫·두 번째 겨울"에서 "4년 안"으로 바꿨다.
