...
# Node 24 LTS for the game (remote runs and the play server). The system /usr/bin/node stays 20 for the DGX's other
# users; only ~/fls-runs/_tools/node is ours. Pinned version and checksum; `node` is a symlink swapped atomically.
NODE_VERSION=24.21.0
NODE_SHA256=6ad1325edbdb5649c379b75a237147a666c95d4f9ae8d340fef2d1575d289ad2
if [ "$("$TOOLS/node/bin/node" -v 2>/dev/null)" != "v$NODE_VERSION" ]; then
  tarball=node-v$NODE_VERSION-linux-arm64.tar.xz
  curl -fsSL -o "$TOOLS/$tarball" "https://nodejs.org/dist/v$NODE_VERSION/$tarball"
  echo "$NODE_SHA256  $TOOLS/$tarball" | sha256sum -c --quiet
  tar -xJf "$TOOLS/$tarball" -C "$TOOLS" && rm -f "$TOOLS/$tarball"
  ln -sfn "node-v$NODE_VERSION-linux-arm64" "$TOOLS/node.new" && mv -Tf "$TOOLS/node.new" "$TOOLS/node"
fi
export PATH="$TOOLS/node/bin:$PATH"
echo "node $(node -v) ($TOOLS/node/bin/node; system $(/usr/bin/node -v 2>/dev/null) untouched)"
