...
#   scripts/remote/run.sh install15 -- bash scripts/remote/with-base-build.sh <trunk-sha> -- bash scripts/install15Verification.sh
...
# the per-frame stages around the pop176 turn here and on the trunk ($BASE_URL), the browser determinism check (same
# state twice, the second from a scene rebuilt in reverse) and save determinism.
set -u
...
node scripts/turnTimeline.mjs "$out/turn-timeline.json" --url "$URL" > "$out/turn-timeline.log" 2>&1; echo "turn-timeline exit $?"
node scripts/turnTimeline.mjs "$out/turn-timeline-trunk.json" --url "$BASE_URL" > "$out/turn-timeline-trunk.log" 2>&1; echo "turn-timeline-trunk exit $?"
