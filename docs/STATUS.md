...
**V1 시각 변형**(Claude Code, 렌더·에셋·테스트만): Wave 2 에셋 31장 설치·대장 31행. 같은 등급의 집(L0~L4·합필 L3/L4)·우물·창고·시장·예배당·방앗간(풍차)·밀밭(혼합형)이 저장된 값만으로 정해지는 seed로 서로 다른 그림을 고른다(등급 풀 안, 계열 선호, 인접 한 번 밀어내기). 하락 33종 오버레이는 변형마다 좌표를 다시 등록했다. 관문 5개 통과: ①화면 SHA·저장→불러오기 변형 동일, ②seed 2·3 등급별 최대 45%·인접 동일 14%/0%, ③오버레이 캡처, ④frameWork 악화 없음, ⑤깨끗한 클론 회귀. 목축형 농장은 보류(C5). 같은 작업에서 **곡선 지면 `RENDER_BOUNDARY_V2` 기본 켬**(사용자 결정, 토글 유지). [V1 보고서](verification/v1-visual-variants/REPORT.md).
