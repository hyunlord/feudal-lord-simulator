...
export function recurringDeliveryRoutes(state: GameState): RecurringDeliveryRoutes {
  ...
  const layout = 'eligible-exits-v1|' + relevant.map(b => `${b.id}:${b.kind}:${b.tx},${b.ty}:${JSON.stringify(buildingFootprint(b))}:${b.workers >= BUILDING_CONFIG_BY_KIND[b.kind].workersRequired}`).join('|');
  ...
  const starts = relevant.filter(b => b.kind === 'granary').map(b => ({ id: b.id, building: b, ready: b.workers >= BUILDING_CONFIG_BY_KIND.granary.workersRequired }));
  const homes = state.houses.flatMap(house => {
    ...
    const providers = starts.flatMap(provider => {
      const edges = feasibleDistributorDistance(state, provider.building, house.buildingId);
      return edges === null || edges > BALANCE.DISTRIBUTOR_RANGE ? [] : [{ id: provider.id, edges }];
    });
