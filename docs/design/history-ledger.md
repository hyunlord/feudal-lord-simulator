...
| 결정 | 대안 | 예측 키 | 예측 |
|---|---|---|---|
| 대기근 대응 | 나머지 3 대응 | population · treasury | 방관 이탈 2가구 · 투기 3가구(가난한 1/4 한도), 구휼 금고 − 구휼 비용(두 계절 안 구휼 계절마다 나눈 빵 × 시장가, 산 빵과 곡창 방출 모두 — [FC-2a](flow-chapter-one.md)) · 투기 + 수입 ÷ 2 |
...
- 실제: 결정 2계절(2,000틱) 뒤 원장이 같은 키로 `actual`을 채운다(`pendingActuals`). 기록 수는 늘지 않는다. 구휼의 금고만은 결정 때 금고 − 그 사이 장부에 적힌 구휼 비용(현금 + 현물)이다(FC-2a).
