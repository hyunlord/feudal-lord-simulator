...
- **목책 지오메트리와 선포**: 정착지 점유 영역을 둘러싼 결정론적 경계를 만들고, 통로·충돌·유효 경로를 검증한다. 선포는 하나의 원자적 이벤트이며 경계 한 칸당 목재 15를 공사 요구량으로 확정한다. 요구 조건인 가용 목재 250만 있으면 전체 성벽 비용이 더 커도 즉시 시작할 수 있어, 지금 착공해 공급 정체를 감수할지 더 모을지 선택하게 된다.
...
아래 이미지는 DGX의 실제 React/reducer/tick/canvas 경로를 Chromium으로 실행해 얻었다. 절반 장면은 정확히 **7/14 구간 완성**과 실제 엔진 정체 `no_material_source`를 함께 검증했고, 공급 경로 복구 후 동일 tick 경로로 **14/14 구간 완성**까지 진행했다.
...
DGX에서 수정 코드 후보 `a22797e2e99a3f17d226b464c13e7bf027d24fad`로 `npm run harness`를 두 번 실행했다.
...
- DGX `npm test`: **634 passed, 0 failed**, 12 suites.
- DGX `npm run typecheck`: PASS.
- DGX `npm run build`: PASS, Vite production bundle 생성.
- DGX 자산 경계 테스트: **20 passed, 0 failed**.
- 실브라우저 판정: 1280/768/375 모두 수평 overflow 0, page error 0, request failure 0. `Escape`, 잘못된 drag, 의식 닫기, reload, tooltip, 7/14 실제 정체, 14/14 완공 검사가 모두 PASS.
- 5× benchmark 3회: 평균 **4.883 / 4.893 / 4.863ms**, p95 **5.6 / 5.5 / 5.4ms**, 최악 **8.1 / 9.6 / 8.4ms**, 12ms 초과 프레임은 모두 0.

## 7. Git과 배포

- 수정 코드 후보: `a22797e2e99a3f17d226b464c13e7bf027d24fad`
- 검토용 캡처 커밋: `4ac81e984657905551d2ad0a250c56cd06308d0b`
...
정확히 250 목재에서 전체 성벽 비용을 선납하지 않고도 시작할 수 있으므로 **지금 착공해 공급 정체를 감수할지, 더 비축해 매끄럽게 올릴지 정하는 선택**이 됐다. 다만 테스트 시나리오는 목재 생산 기반이 이미 안정적이어서 실제 플레이 체감은 여전히 점등 직후 누르는 버튼 쪽에 가깝다. 40% 노동 전환과 공급 부족은 실재하지만 경쟁하는 긴급 목표는 아직 약하다.
