...
| 대응 | 계절 시작마다 | 사다리 | 장부 |
|---|---|---|---|
| 구휼 `relief` | 가난한 가구의 한 계절 빵을 나눈다. 먼저 기근 값으로 사서 곡창에 넣고(지난 계절 현금 수입과 금고의 50 % 가운데 적은 것까지 — 기근이 저축이 아니라 벌이를 먹는다), 모자라면 곡창의 빵을 방출한다(FC-2a). 가난한 가구는 부족이 아니다 | 이탈 계절당 1 | `famine_relief`(현금 지출 + 현물 방출, 출처 사건) |
...
## FC-2a 구휼 비용 (FIX-2, `famineRelief.ts`)

구휼 비용 = 나눈 빵 × 시장가. 산 빵은 현금으로, 곡창에서 방출한 빵은 방출 가치로 센다. 결정 FC12.

- **한 계절의 구휼**(도래 중 계절 시작마다, `reliefSeason`): 나눌 빵 `need` = 가난한 가구(FC-2의 1/4)의 한 계절 빵(배급량).
  - ① 현금으로 먼저 산다: `bought = min(need, ⌊예산 ÷ 시장가⌋, 첫 곡창의 빈 칸)`, 예산 = min(지난 계절 현금 수입, 금고의 50 %)(FC3). 산 빵은 첫 곡창에 들어간다. 장부: `famine_relief`, 현금, −bought × 시장가, 출처 사건 + 곡창.
  - ② 모자란 몫은 곡창의 빵에서 방출한다: `released = min(need − bought, 곡창들의 빵)`. 방출 가치는 그 빵을 지금 시장에서 팔면 받을 값(시장가, 투기가 파는 값과 같다)이다. 장부: `famine_relief`, 현물(`in_kind`, resource `bread`), −방출 × 시장가, 곡창마다 한 줄(id 순), 출처 사건 + 곡창.
  - 현물 항목은 돈을 움직이지 않는다(금고·계절 수지는 현금만 센다). 방출한 빵은 따로 옮기지 않고 여느 때처럼 마을 거래로 나간다. 그래서 시뮬레이션은 바뀌지 않고, 장부에 현물 항목이 더해질 뿐이다.
  - 사지도 방출하지도 못하면(수입 0 ∧ 곡창에 빵 0) 나눈 빵이 없고, 비용도 0이다.
- **예측**(HL-3, `reliefCostForecast`): 결정 때의 수치로, 결정 뒤 두 계절(due)까지 남은 구휼 계절(결정 < 계절 시작 ≤ due, 기근 도래 중)마다 ①②를 되풀이한다. 금고는 앞 계절에 산 값만큼, 곡창의 빵은 앞 계절에 방출한 만큼 줄여 다음 계절을 나눈다. 예측 금고 = 지금 금고 − 비용 합.
- **실제**(HL-3, `reliefCostPosted`): 결정 때 금고 − (결정, due] 사이 그 사건의 `famine_relief` 항목(현금 + 현물) 합. 결정 기록 params에 `eventId`·`treasuryAtDecision`을 둔다. 이 둘이 없는 옛 기록(FIX-2 전 저장)은 전처럼 due 때의 금고를 읽는다.
- 수입이 0이어도 곡창에 빵이 있으면 예측과 실제 모두 금고가 줄어든다. "그대로"는 나눈 빵이 없을 때만 나온다.
