...
- **LB-11 성 안이 될 땅**: 목책 전(촌락)에는 자동 성장이 경작지 2×2 블록을 지금 중심에 제안될 성벽에서 한 칸(`fieldWallMargin`) 떨어진 곳에 먼저 칠한다. 그런 블록이 없을 때만 AF-13 탐색 그대로 칠한다. 밭이 목책 안 필지 땅을 차지해 필지가 17에서 멈추던 seed 3 교착(관문 ② 회귀)을 막는다. 결정 M12-R04(성벽 안 대형 밀밭 금지, 권고)와 같은 방향이다.
