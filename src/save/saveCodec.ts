...
export function decodeSave(bytes: Uint8Array): DecodedSave {
  ...
  if (fromVersion === SAVE_SCHEMA_VERSION && envelope.checksum !== undefined) {
    const markerAt = text.indexOf(STATE_MARKER);
    const stateJson = markerAt < 0 ? JSON.stringify(envelope.state) : text.slice(markerAt + STATE_MARKER.length, -1);
    const actual = stateChecksum(stateJson);
    if (actual !== envelope.checksum) {
      throw new SaveChecksumError(`Save checksum mismatch: expected ${envelope.checksum}, got ${actual}`);
    }
  }
