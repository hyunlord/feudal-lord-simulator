...
/** Cleared forest cells (forestHarvests), cached on that array (replaced when a tile is logged). */
const clearedCache = new WeakMap<object, ReadonlySet<number>>();
export function clearedCells(state: Pick<GameState, "forestHarvests" | "width">): ReadonlySet<number> {
  const harvests = state.forestHarvests ?? [];
  const cached = clearedCache.get(harvests);
  if (cached !== undefined) return cached;
  const cells = new Set(harvests.map(harvest => harvest.ty * state.width + harvest.tx));
  clearedCache.set(harvests, cells);
  return cells;
}
...
export function buildZoneLayer(state: GameState, cells: readonly (Tile | undefined)[]): ZoneLayer {
  ...
  const fields = zones.map(zone => zone.kind !== "arable" ? null : arableField({ zoneId: zone.id, zoneOrdinal: zone.createdOrdinal,
    layout: arableStripStates(zone, state), mapWidth: width, mapHeight: height, cells, seed: state.seed, placedStamps, cleared: clearedCells(state) }));
